...
contract SpeczToken is ERC20Detailed, Ownable, MinterRole {
    ...
    uint256 public Treasury = 1;
    uint256 public RiskFreeValue = 1;
    ...
    constructor(string memory name, string memory id) ERC20Detailed(name, id, uint8(DECIMALS)) {}

    function setRebaseRate(uint256 rebaseRate) public onlyOwner {
    ...
    function initialise(
        ...
        autoLiquidityReceiver = alr;
        TreasuryReceiver = msg.sender;
        RiskFreeValueReceiver = rfvr;
    ...
    function rebase() public returns (bool) {
        if (epoch.endTime <= uint32(block.timestamp)) {
            ...
            uint256 supplyDelta = _totalSupply / uint256(_rebaseRate / 10**DECIMALS);
            
            require(!inSwap, "Try again");
            if (supplyDelta == 0) {
                emit LogRebase(epoch.number, _totalSupply);
                return false;
            }
    ...
    /**
        @notice Method is strickly for emergency events or risk mitigtion scenarios.
        Max fees have been hard coded into the contract to mitigate HP concerns.
     */
    function setFees(
        ...
        require(_liquidityFee <= 10);
        require(_sellFee <= 10);
        require(_Treasury <= 6);
        require(_RiskFreeValue <= 10);
